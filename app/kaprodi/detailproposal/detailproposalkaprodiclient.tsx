...
const fetchDetailProposalData = async (proposalId: string | null) => {
  ...
  const [ { data: supervisors }, { data: recommendations } ] = await Promise.all([
    supabase.from("thesis_supervisors").select(`dosen_id, role, status, rejection_reason, is_layak, dosen:profiles ( nama )`).eq("proposal_id", proposalId),
...
export default function DetailProposalKaprodiClient() {
  ...
  const handleDosenResponse = async (isAccepted: boolean) => {
    ...
    try {
      ...
      const { data: myProfile } = await supabase.from("profiles").select("nama").eq("id", currentUserId).single();
      const namaDosen = myProfile?.nama || "Dosen";
      ...
      if (existing) {
        ...
        const { error } = await supabase.from("thesis_supervisors").insert({ 
          proposal_id: proposalId, dosen_id: currentUserId, role: myRole, ...payloadSup 
        });
        if (error) throw error;
      }
      ...
      if (!isAccepted) {
        if (isLayak === false) {
          const { error: propErr } = await supabase.from("proposals").update({ status: "Ditolak Dosbing", is_locked: false }).eq("id", proposalId);
          if (propErr) throw propErr; 
          
          if (proposal?.user?.id) {
            await sendNotification(proposal.user.id, "Proposal Ditolak", `Proposal Anda dinilai TIDAK LAYAK oleh ${namaDosen} dengan alasan: "${rejectReason}". Anda diwajibkan untuk merevisi dan mengunggah dokumen proposal baru.`);
          }
        } else {
          if (proposal?.user?.id) {
            await sendNotification(proposal.user.id, "Perubahan Dosen Pembimbing", `Dosen ${namaDosen} tidak dapat membimbing Anda karena: "${rejectReason}". Namun proposal Anda dinilai LAYAK. Kaprodi akan mencarikan Dosen Pembimbing pengganti.`);
          }
        }
      } else {
      ...
      alert(isAccepted ? "Berhasil menyetujui" : "Berhasil menolak proposal!");
              ...
              <div className="flex items-center gap-8 relative z-10">
                <div className="w-28 h-28 bg-slate-100 rounded-[2.5rem] flex items-center justify-center border-4 border-white shadow-xl overflow-hidden shrink-0 relative">
                  {proposal.user?.avatar_url ? (
                    <Image src={proposal.user.avatar_url} alt={proposal.user?.nama || "User"} layout="fill" sizes="(max-width: 768px) 100vw, 150px" className="object-cover" />
                  ) : (
              ...
              <div className={`rounded-[2rem] p-8 text-white shadow-xl transition-all duration-300 ${myResponseData.status === 'accepted' ? 'bg-emerald-600 shadow-emerald-100' : myResponseData.status === 'rejected' ? 'bg-red-600 shadow-red-100' : 'bg-blue-600 shadow-blue-100'}`}>
                ...
                {myResponseData.status === 'rejected' && (
                  <div className="mb-6 space-y-2">
                    <div className="bg-red-800/50 p-3 rounded-xl">
                      <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mb-1">Catatan Penolakan:</p>
                      <p className="italic text-xs font-medium leading-relaxed">"{myResponseData.rejection_reason}"</p>
                    </div>
                    <div className={`p-3 rounded-xl flex items-center gap-2 ${myResponseData.is_layak ? 'bg-emerald-500/30' : 'bg-red-800/50'}`}>
                       {myResponseData.is_layak ? <CheckCircle size={14} /> : <XCircle size={14} />}
                       <p className="text-[10px] font-black uppercase tracking-widest">{myResponseData.is_layak ? 'Proposal Layak (Hanya Ganti Dosen)' : 'Proposal Tidak Layak (Wajib Revisi)'}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="mb-8 space-y-4">
                  ...
                  {allSupervisorsStatus.map((s: any, idx: number) => (
                    <div key={idx} className="flex flex-col p-4 bg-slate-50 rounded-2xl border border-slate-100 transition-all hover:border-blue-100">
                      ...
                      {s.status === 'rejected' && (
                        <div className="mt-2 p-3 bg-red-50/50 border border-red-100 rounded-xl flex flex-col gap-2 items-start">
                          <div className="flex gap-2">
                             <XCircle size={14} className="text-red-400 shrink-0 mt-0.5" />
                             <p className="text-[10px] text-red-600 font-medium italic leading-relaxed">"{s.rejection_reason || 'Tidak ada alasan spesifik'}"</p>
                          </div>
                          <span className={`w-fit text-[9px] font-black uppercase px-2.5 py-1 rounded-lg ${s.is_layak ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                             {s.is_layak ? 'Status: Substansi Layak' : 'Status: Tidak Layak'}
                          </span>
                        </div>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in duration-200">
            <div className="p-10 text-center">
              ...
              <p className="text-slate-500 text-xs mb-4 font-medium">Berikan alasan Anda tidak dapat membimbing.</p>
              
              <textarea 
                value={rejectReason} 
                onChange={(e) => setRejectReason(e.target.value)} 
                placeholder="Misal: Kuota bimbingan saya sudah penuh..." 
                className="w-full h-28 p-5 bg-slate-50 border border-slate-200 rounded-[1.5rem] font-bold text-sm outline-none focus:ring-4 focus:ring-red-50 transition-all placeholder:text-slate-300 resize-none" 
              />

              <div className="mt-6 text-left">
                <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 text-center">Apakah substansi proposal ini layak dijadikan skripsi?</label>
                <div className="flex gap-3">
                  <button
                    onClick={() => setIsLayak(true)}
                    className={`flex-1 py-4 px-4 rounded-xl border flex flex-col items-center justify-center gap-2 transition-all active:scale-95 ${isLayak === true ? 'bg-emerald-50 border-emerald-300 text-emerald-600 ring-4 ring-emerald-100 shadow-sm' : 'bg-white border-slate-200 text-slate-400 hover:bg-slate-50'}`}
                  >
                    <CheckCircle size={20} /> <span className="text-[10px] font-black uppercase tracking-widest">Ya, Layak</span>
                  </button>
                  <button
                    onClick={() => setIsLayak(false)}
                    className={`flex-1 py-4 px-4 rounded-xl border flex flex-col items-center justify-center gap-2 transition-all active:scale-95 ${isLayak === false ? 'bg-red-50 border-red-300 text-red-600 ring-4 ring-red-100 shadow-sm' : 'bg-white border-slate-200 text-slate-400 hover:bg-slate-50'}`}
                  >
                    <XCircle size={20} /> <span className="text-[10px] font-black uppercase tracking-widest">Tidak Layak</span>
                  </button>
                </div>
                <p className="text-[9px] text-center text-slate-400 mt-4 px-2 leading-relaxed">
                  {isLayak === true && "Mahasiswa tidak perlu upload ulang. Kaprodi akan menetapkan dosen pengganti."}
                  {isLayak === false && "Mahasiswa akan diwajibkan melakukan revisi dan unggah ulang dokumen proposal."}
              ...
              <div className="flex gap-4 mt-8 pt-6 border-t border-slate-100">
                ...
                <button 
                  ...
                  disabled={!rejectReason.trim() || isLayak === null || loading}
