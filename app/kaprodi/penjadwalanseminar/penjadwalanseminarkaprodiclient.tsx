...
const fetchJadwalSeminarData = async (requestId: string | null) => {
  ...
  if (finalData.proposal?.docs) {
    ...
    if (formJadwal?.file_url) {
      ...
      formJadwalUrl = signedDoc?.signedUrl ?? null;
    }
  }
  ...
  const [ { data: existingSchedule }, { data: examinerData } ] = await Promise.all([
    supabase.from('seminar_schedules').select('tanggal, jam, ruangan').eq('seminar_request_id', requestId).maybeSingle(),
    supabase.from('examiners').select('dosen_id, role').eq('seminar_request_id', requestId)
  ]);

  const p1 = examinerData?.find(e => e.role === 'penguji1')?.dosen_id || "";
  const p2 = examinerData?.find(e => e.role === 'penguji2')?.dosen_id || "";
  const p3 = examinerData?.find(e => e.role === 'penguji3')?.dosen_id || "";
...
export default function PenjadwalanSeminarKaprodiClient() {
  ...
  const { data: cache, isLoading, mutate } = useSWR(
    requestId ? `penjadwalan_seminar_${requestId}` : null,
    () => fetchJadwalSeminarData(requestId),
    {
  ...
  const studentData = cache?.studentData || null;
  const fileUrl = cache?.fileUrl || null;
  const formJadwalUrl = cache?.formJadwalUrl || null;
  ...
  useEffect(() => {
    if (cache) {
      if (cache.existingSchedule) {
        setAlreadyScheduled(true);
        setTanggal(cache.existingSchedule.tanggal ?? "");
        setJam(cache.existingSchedule.jam ?? "");
        setRuangan(cache.existingSchedule.ruangan ?? "");
      }
  ...
  const handleTetapkanJadwal = async () => {
    ...
    try {
      ...
      const { error: deleteExamError } = await supabase.from('examiners').delete().eq('seminar_request_id', requestId);
      ...
      const { error: updateError } = await supabase.from('seminar_requests').update({ status: 'Disetujui' }).eq('id', requestId);
      if (updateError) throw updateError;

      const studentId = studentData.proposal?.user_id;
      const mhsName = studentData.proposal?.user?.nama || "Mahasiswa";
      const formattedDate = new Date(tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });

      const notificationPromises = [];

      if (studentId) {
        notificationPromises.push(sendNotification(studentId, "Jadwal Seminar Ditetapkan", `Jadwal seminar Anda telah ditetapkan pada ${formattedDate} pukul ${jam} WIB di ${ruangan}.`));
      }

      const supervisorIds = studentData.proposal?.supervisors.map(s => s.dosen_id) || [];
      supervisorIds.forEach(dosbingId => {
        if (dosbingId) {
          notificationPromises.push(sendNotification(dosbingId, "Jadwal Seminar Bimbingan", `Kaprodi telah menetapkan jadwal seminar mahasiswa bimbingan Anda atas nama ${mhsName} pada ${formattedDate}, pukul ${jam} WIB di ${ruangan}.`));
        }
      });

      const examinerIds = [penguji1, penguji2, penguji3];
examinerIds.forEach(pengujiId => {
  if (pengujiId) {
    notificationPromises.push(
      sendNotification(
        pengujiId,
        "Tugas Menguji Seminar",
        `Kaprodi telah menetapkan jadwal Anda untuk menguji seminar mahasiswa atas nama ${mhsName} pada ${formattedDate}, pukul ${jam} WIB di ${ruangan}.`
      )
    );
  }
});

const { data: tendikList } = await supabase
  .from('profiles')
  .select('id')
  .eq('role', 'tendik');
...
if (tendikList && tendikList.length > 0) {
  ...
  const { data: namaPenguji } = await supabase
    .from('profiles')
    .select('nama')
    .in('id', [penguji1, penguji2, penguji3]);

  const listNamaPenguji =
    namaPenguji?.map((n) => n.nama).join(', ') || "-";

  tendikList.forEach((tendik) => {
    notificationPromises.push(
      sendNotification(
        tendik.id,
        "Jadwal Seminar Ditetapkan",
        `Mahasiswa atas nama ${mhsName} dengan judul "${activeProposal.judul}" telah ditetapkan seminar pada tanggal ${formattedDate} pukul ${jam} WIB di ${ruangan}. Dosen penguji: ${listNamaPenguji}.`
      )
    );
  });
}
...
await Promise.all(notificationPromises);
  ...
  const pUtama = getSupervisorObj('utama') || getSupervisorObj('pembimbing1');
  const pPendamping = getSupervisorObj('pendamping') || getSupervisorObj('pembimbing2');
    ...
    <div className="min-h-screen bg-[#F8F9FB] flex flex-col font-sans text-slate-700">
      <main className="flex-1 p-10 max-w-[1400px] mx-auto w-full">
       ...
       <div className="mb-10 outline-none focus:outline-none">
          <Link href="/kaprodi/pengajuanseminar" className="group flex items-center gap-4 mb-8 w-fit transition-all active:scale-95 outline-none focus:outline-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-white shadow-xl shadow-slate-200/50 overflow-hidden relative group">
              <div className="absolute top-0 right-0 p-10 opacity-[0.03] group-hover:opacity-[0.05] transition-opacity"><User size={180} /></div>
              <div className="p-10 flex items-center gap-8 relative z-10">
                <div className="w-28 h-28 bg-slate-100 rounded-[2.5rem] flex items-center justify-center border-4 border-white shadow-xl shrink-0 overflow-hidden relative">
                  {activeProposal.user?.avatar_url ? (
                    <Image src={activeProposal.user.avatar_url} alt={activeProposal.user.nama || "User"} layout="fill" objectFit="cover" />
                  ) : (
                    <User size={56} className="text-slate-300" />
                  )}
                </div>
                <div>
                  <h2 className="text-2xl font-black text-slate-800 tracking-tight leading-none">{activeProposal.user?.nama}</h2>
              ...
              <div className="px-10 py-8 bg-slate-50/50 border-t border-slate-100">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <FileText size={14} className="text-blue-500" /> Judul Skripsi 
                </p>
                <p className="text-xl font-bold text-slate-700 leading-relaxed">"{activeProposal.judul}"</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100 border-t border-slate-100 bg-white">
                <div className="p-8 flex items-center gap-5">
                  <div className="w-14 h-14 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 border border-indigo-100 relative overflow-hidden">
                    {pUtama?.dosen?.avatar_url ? <Image src={pUtama.dosen.avatar_url} alt="P1" layout="fill" objectFit="cover" /> : <User size={24} />}
                  </div>
                ...
                <div className="p-8 flex items-center gap-5">
                  <div className="w-14 h-14 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0 border border-emerald-100 relative overflow-hidden">
                     {pPendamping?.dosen?.avatar_url ? <Image src={pPendamping.dosen.avatar_url} alt="P2" layout="fill" objectFit="cover" /> : <User size={24} />}
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Co-pembimbing</p>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between bg-blue-50/50 border border-blue-100 p-6 rounded-2xl gap-4">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                     ...
                     <p className="text-[10px] font-bold text-slate-400 tracking-widest leading-relaxed">Berisi kesepakatan waktu yang diajukan oleh mahasiswa dan dosen pembimbing.</p>
                   </div>
                </div>
                {formJadwalUrl ? (
                   <a 
