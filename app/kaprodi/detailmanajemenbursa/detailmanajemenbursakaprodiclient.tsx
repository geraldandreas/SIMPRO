...
export default function DetailManajemenBursa() {
  ...
  const bimbinganAktif = supervisors.filter((s: any) => {
    const p = s.proposal;
  ...
  const riwayatKeputusan = supervisors.filter((s: any) => s.status === "rejected"); 

  const antreanDisetujuiDosen = supervisors.filter((s: any) => 
