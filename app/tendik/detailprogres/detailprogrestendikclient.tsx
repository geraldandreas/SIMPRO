...
const PROPOSAL_DOCS = [
  { id: 'file_transkrip', statusKey: 'status_file_transkrip', label: "Transkrip Nilai & KRS Ter-ACC Dosen Wali" },
  { id: 'file_formulir', statusKey: 'status_file_formulir', label: "Formulir Kesanggupan Menyelesaikan TA" },
  { id: 'file_referensi', statusKey: 'status_file_referensi', label: "File Dokumen Referensi Jurnal Utama" },
];
...
const SIDANG_DOCS = [
  { id: 'form_pengajuan_sidang', label: "Formulir Pengajuan Sidang Tugas Akhir", type: 'online' },
  { id: 'ba_bimbingan', label: "Berita Acara Bimbingan (TTD Pembimbing)", type: 'online' },
  { id: 'matriks_perbaikan', label: "Formulir Matriks Perbaikan Skripsi (TTD Pembimbing & Penguji)", type: 'online' },
  { id: 'sertifikat_toefl', label: "Sertifikat TOEFL (Skor minimal 475)", type: 'online' },
  { id: 'bebas_perpus', label: "Bebas Pinjaman Perpustakaan (Univ & Fak)", type: 'online' },
  { id: 'jurnal_skripsi', label: "Jurnal Skripsi (Maksimal 10 halaman)", type: 'online' },
  { id: 'skpi_screenshot', label: "Screenshot SKPI SIAT Students", type: 'online' },
  { id: 'biodata_sidang', label: "Biodata Sidang Diserta Ijazah SMA & Berkas Akte", type: 'online' },
  { id: 'foto_sidang', label: "Foto Hitam Putih Cerah ukuran 2x3", type: 'online' },
  { id: 'skripsi_tugas_akhir', label: "Skripsi Tugas Akhir (File)", type: 'online' },
  { id: 'flyer_skripsi', label: "Flyer Skripsi Tugas Akhir (File)", type: 'online' },
  { id: 'print_skripsi', label: "Print Out Skripsi 5 buah belum dijilid", type: 'offline' },
  { id: 'flyer_a3', label: "Flyer Skripsi ukuran A3 (Cetak)", type: 'offline' },
];
...
const fetchDetailTendikData = async (proposalId: string | null) => {
  ...
  let proposalUrls: Record<string, string | null> = {};
  const proposalDocsRaw: Record<string, string | null> = {
    file_path: propData.file_path,
    file_transkrip: propData.file_transkrip,
    file_formulir: propData.file_formulir,
    file_referensi: propData.file_referensi
  };

  const proposalStatuses: Record<string, string> = {
    status_file_proposal: propData.status_file_proposal,
    status_file_transkrip: propData.status_file_transkrip,
    status_file_formulir: propData.status_file_formulir,
    status_file_referensi: propData.status_file_referensi
  };

  await Promise.all(Object.entries(proposalDocsRaw).map(async ([key, path]) => {
    if (path) {
      const { data: sData } = await supabase.storage.from("proposals").createSignedUrl(path, 3600);
      if (sData?.signedUrl) proposalUrls[key] = sData.signedUrl;
    }
  }));
...
export default function DetailProgresTendikClient() {
  ...
  const proposalStatuses = cache?.proposalStatuses || {};
  ...
  const isProposalVerifiedKaprodi = ['Diterima', 'Ditetapkan', 'Lulus'].includes(student?.status_proposal || '');

  const [processingDoc, setProcessingDoc] = useState<string | null>(null);
  const [activeDropdownId, setActiveDropdownId] = useState<string | null>(null);
  const [isRejectModalOpen, setIsRejectModalOpen] = useState(false);
  const [docToReject, setDocToReject] = useState<{id: string, category: 'proposal' | 'seminar' | 'sidang', label: string} | null>(null);
  ...
  const handleActionFile = async (rawPath: string, bucketName: string, download = false) => {
    if (!rawPath) return;
    try {
      const cleanPath = bucketName === 'docseminar' ? normalizeStoragePath(rawPath) : rawPath;
  ...
  const handleOpenRejectModal = (docId: string, category: 'proposal' | 'seminar' | 'sidang', label: string) => {
  ...
  const handleVerify = async (docId: string, newStatus: string, category: 'proposal' | 'seminar' | 'sidang', reason?: string) => {
    ...
    try {
      ...
      if (category === 'proposal') {
        const docObj = PROPOSAL_DOCS.find(d => d.id === docId);
        if (!docObj) return;

        await supabase.from('proposals').update({ [docObj.statusKey]: newStatus }).eq('id', proposalId);
        const { data: latest } = await supabase.from('proposals').select('status_file_transkrip, status_file_formulir, status_file_referensi').eq('id', proposalId).single();
        
        if (latest) {
          const allStatuses = [latest.status_file_transkrip, latest.status_file_formulir, latest.status_file_referensi];
          const isAnyRejected = allStatuses.includes('Ditolak');
          const completeCount = allStatuses.filter(s => s === 'Lengkap').length;

          if (isAnyRejected) {
             await supabase.from('proposals').update({ status: 'Ditolak Tendik', is_locked: false }).eq('id', proposalId);
             if (student?.user.id) {
               await sendNotification(student.user.id, "Berkas Administrasi Ditolak", `Dokumen "${docObj.label}" Anda DITOLAK. Alasan: "${reason}". Silakan perbaiki dan unggah ulang.`);
             }
          } else if (completeCount === PROPOSAL_DOCS.length) {
             await supabase.from('proposals').update({ status: 'Menunggu Persetujuan Dosbing', is_locked: true }).eq('id', proposalId);
             if (student?.user.id) {
               await sendNotification(student.user.id, "Verifikasi Administrasi Berhasil", "Seluruh berkas persyaratan proposal Anda telah divalidasi. Saat ini menunggu penetapan dari dosen pembimbing.");
             }
             const { data: kaprodi } = await supabase.from('profiles').select('id').eq('role', 'kaprodi').maybeSingle();
             if (kaprodi) {
               await sendNotification(kaprodi.id, "Usulan Judul Baru", `${student?.user.nama} telah mengajukan judul proposal baru: "${student?.judul}".`);
             }
             const { data: recs } = await supabase.from('proposal_recommendations').select('dosen_id').eq('proposal_id', proposalId);
             if (recs && recs.length > 0) {
               for (const rec of recs) {
                 await sendNotification(rec.dosen_id, "Pengajuan Calon Pembimbing", `Mahasiswa atas nama ${student?.user.nama} mengajukan Anda sebagai calon pembimbing.`);
               }
             }
          } else {
             if (student?.user.id && newStatus === 'Lengkap') {
                 await sendNotification(student.user.id, "Berkas Terverifikasi", `Dokumen "${docObj.label}" Anda telah divalidasi oleh Tendik.`);
             }
          }
        }
        mutate();
      ...
      if (student?.user.id) {
        ...
        let message = newStatus === 'Lengkap' 
          ...
          : `Dokumen "${docLabel}" Anda DITOLAK. Alasan: "${reason}". Silakan perbaiki kembali berkas yang benar.`;
  ...
  const validProposalCount = PROPOSAL_DOCS.filter(reqDoc => proposalStatuses[reqDoc.statusKey] === 'Lengkap').length;
  const proposalPercent = Math.min(100, Math.round((validProposalCount / PROPOSAL_DOCS.length) * 100));
  ...
  const totalValid = validProposalCount + validSeminarCount + validSidangCount;
  const totalDocs = PROPOSAL_DOCS.length + REQUIRED_DOCS.length + SIDANG_DOCS.length;
          ...
          <div className="lg:col-span-8 space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-8 rounded-[2.5rem] border border-white shadow-xl shadow-slate-200/40 flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-xl font-black text-slate-800">{totalValid} / {totalDocs} Berkas</p>
                </div>
              </div>
              <div className="bg-white p-8 rounded-[2.5rem] border border-white shadow-xl shadow-slate-200/40 flex items-center gap-6">
            ...
            <div className="bg-white rounded-[3rem] border border-white shadow-2xl shadow-slate-200/40 overflow-visible">
              <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
                <h3 className="text-lg font-black text-slate-800 uppercase tracking-tighter">Verifikasi Administrasi Proposal</h3>
                <div className="flex items-center gap-3">
                  <span className="text-[10px] font-black text-slate-400">Progres</span>
                  <div className="w-32 h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-600 transition-all duration-1000" style={{ width: `${proposalPercent}%` }} />
                  </div>
                  <span className="text-xs font-black text-blue-600">{proposalPercent}%</span>
                </div>
              </div>
              <div className="divide-y divide-slate-50">
                <div className="flex items-center justify-between p-6 bg-slate-50/30 border-b border-slate-50">
                  <div className="flex items-center gap-5">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center transition-all bg-indigo-50 text-indigo-500">
                      <FileText size={24} />
                    </div>
                    <div>
                      <p className="text-sm font-black text-slate-700 tracking-tight">Dokumen Proposal Skripsi (PDF)</p>
                      <div className="flex items-center gap-2 mt-1.5">
                        {isProposalVerifiedKaprodi ? (
                          <span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md text-emerald-600 bg-emerald-50 border border-emerald-100">
                            VERIFIED ( PEMBIMBING SUDAH DITETAPKAN)
                          </span>
                        ) : (
                          <span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md text-indigo-600 bg-indigo-50 border border-indigo-100">
  ...
  {cache?.proposalDocsRaw?.file_path && (
    <button 
      onClick={() => handleActionFile(cache.proposalDocsRaw.file_path as string, 'proposals', false)} 
      className="p-3 rounded-xl border bg-white text-slate-400 border-slate-200 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50/50 transition-all active:scale-95 flex items-center gap-2"
    >
      <Eye size={16} /> 
      <span className="text-[10px] font-black uppercase tracking-widest hidden sm:inline">Lihat</span>
    </button>
  )}
</div>
                </div>

                {PROPOSAL_DOCS.map((doc) => {
                  const rawPath = cache?.proposalDocsRaw?.[doc.id];
                  const hasFile = !!rawPath;
                  
                  const status = hasFile ? (proposalStatuses[doc.statusKey] || 'Belum Diverifikasi') : 'Belum Diserahkan';
                  
                  const isComplete = status === 'Lengkap';
                  const isRejected = status === 'Ditolak';
                  const dropdownKey = `proposal_${doc.id}`;
                    ...
                    <div key={doc.id} className="flex items-center justify-between p-6 hover:bg-blue-50/20 transition-all group relative">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            {isProcessing ? (<span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md text-blue-600 bg-blue-50">Memproses...</span>) : hasFile ? (<span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md ${isComplete ? 'text-emerald-600 bg-emerald-50' : isRejected ? 'text-red-600 bg-red-50' : 'text-amber-600 bg-amber-50'}`}>{isComplete ? 'Verified' : isRejected ? 'Rejected' : 'Pending'}</span>) : (<span className="text-[9px] font-black text-slate-500 uppercase tracking-widest px-2.5 py-1 rounded-md bg-slate-100">Belum Diverifikasi</span>)}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 dropdown-container">
                          <div className="relative">
                            <button onClick={() => setActiveDropdownId(activeDropdownId === dropdownKey ? null : dropdownKey)} disabled={isProcessing || !hasFile} className={`p-3 rounded-xl border transition-all ${isProcessing || !hasFile ? 'opacity-50 cursor-not-allowed' : ''} ${activeDropdownId === dropdownKey ? 'bg-blue-600 border-blue-600 text-white shadow-xl' : 'bg-white text-slate-300 border-slate-100 hover:border-blue-400 hover:text-blue-600'}`}><MoreHorizontal size={20} /></button>
                            {activeDropdownId === dropdownKey && (
                              <div className="absolute right-0 top-12 w-60 bg-white rounded-[1.5rem] shadow-2xl border border-slate-50 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                <div className="p-2 space-y-1">
                                  {hasFile && (
                                    <>
                                      <button onClick={() => handleActionFile(rawPath, 'proposals', false)} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-slate-600 hover:bg-slate-50 rounded-xl transition uppercase tracking-widest"><Eye size={16} className="text-slate-400" /> Lihat Berkas</button>
                                      <button onClick={() => handleActionFile(rawPath, 'proposals', true)} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-slate-600 hover:bg-slate-50 rounded-xl transition uppercase tracking-widest"><Download size={16} className="text-slate-400" /> Unduh PDF</button>
                                      <div className="h-px bg-slate-50 my-1 mx-2" />
                                    </>
                                  )}
                                  <button onClick={() => handleVerify(doc.id, 'Lengkap', 'proposal')} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition uppercase tracking-widest"><CheckCircle size={16} /> Verifikasi Valid</button>
                                  <button onClick={() => handleOpenRejectModal(doc.id, 'proposal', doc.label)} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-red-600 hover:bg-red-50 rounded-xl transition uppercase tracking-widest"><XCircle size={16} /> Tolak / Batal</button>
                                </div>
                              </div>
                            )}
                          </div>
            ...
            <div className="bg-white rounded-[3rem] border border-white shadow-2xl shadow-slate-200/40 overflow-visible">
              ...
              <div className="divide-y divide-slate-50">
                {REQUIRED_DOCS.map((doc) => {
                  ...
                  const isProcessing = processingDoc === dropdownKey;
                  const isOnline = doc.type === 'online';
                    ...
                    <div key={doc.id} className="flex items-center justify-between p-6 hover:bg-blue-50/20 transition-all group relative">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <p className="text-sm font-black text-slate-700 tracking-tight">{doc.label}</p>
                          <div className="flex items-center gap-2 mt-1.5">
                            {isProcessing ? (<span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md text-blue-600 bg-blue-50">Memproses...</span>) : data && data.status !== 'Belum Diserahkan' ? (<span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md ${data.status === 'Lengkap' ? 'text-emerald-600 bg-emerald-50' : data.status === 'Ditolak' ? 'text-red-600 bg-red-50' : 'text-amber-600 bg-amber-50'}`}>{data.status === 'Lengkap' ? 'Verified' : data.status === 'Ditolak' ? 'Rejected' : 'Pending'}</span>) : (<span className="text-[9px] font-black text-slate-500 uppercase tracking-widest px-2.5 py-1 rounded-md bg-slate-100">{isOnline ? 'Belum Diunggah' : 'Pemberkasan Fisik'}</span>)}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 dropdown-container">
                          <div className="relative">
                            <button onClick={() => setActiveDropdownId(activeDropdownId === dropdownKey ? null : dropdownKey)} disabled={isProcessing || (isOnline && !hasFile)} className={`p-3 rounded-xl border transition-all ${isProcessing || (isOnline && !hasFile) ? 'opacity-50 cursor-not-allowed' : ''} ${activeDropdownId === dropdownKey ? 'bg-blue-600 border-blue-600 text-white shadow-xl' : 'bg-white text-slate-300 border-slate-100 hover:border-blue-400 hover:text-blue-600'}`}><MoreHorizontal size={20} /></button>
                            {activeDropdownId === dropdownKey && (
                              <div className="absolute right-0 top-12 w-60 bg-white rounded-[1.5rem] shadow-2xl border border-slate-50 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                <div className="p-2 space-y-1">
                                  {hasFile && isOnline && (
                                    <>
                                      <button onClick={() => handleActionFile(data.file_url, 'docseminar', false)} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-slate-600 hover:bg-slate-50 rounded-xl transition uppercase tracking-widest"><Eye size={16} className="text-slate-400" /> Lihat Berkas</button>
                                      <button onClick={() => handleActionFile(data.file_url, 'docseminar', true)} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-slate-600 hover:bg-slate-50 rounded-xl transition uppercase tracking-widest"><Download size={16} className="text-slate-400" /> Unduh PDF</button>
                                      <div className="h-px bg-slate-50 my-1 mx-2" />
                                    </>
                                  )}
            ...
            <div className="bg-white rounded-[3rem] border border-white shadow-2xl shadow-slate-200/40 overflow-visible">
              ...
              <div className="divide-y divide-slate-50">
                {SIDANG_DOCS.map((doc) => {
                  ...
                  const isProcessing = processingDoc === dropdownKey;
                  const isOnline = doc.type === 'online';
                  const disableAction = isProcessing || (isOnline && !hasFile);
                    ...
                    <div key={doc.id} className="flex items-center justify-between p-6 hover:bg-blue-50/20 transition-all group relative">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <p className="text-sm font-black text-slate-700 tracking-tight">{doc.label}</p>
                          <div className="flex items-center gap-2 mt-1.5">
                            {isProcessing ? (<span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md text-blue-600 bg-blue-50">Memproses...</span>) : data && status !== 'Belum Diserahkan' ? (<span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md ${isComplete ? 'text-emerald-600 bg-emerald-50' : isRejected ? 'text-red-600 bg-red-50' : 'text-amber-600 bg-amber-50'}`}>{isComplete ? 'Verified' : isRejected ? 'Rejected' : 'Pending'}</span>) : (<span className="text-[9px] font-black text-slate-500 uppercase tracking-widest px-2.5 py-1 rounded-md bg-slate-100">{isOnline ? 'Belum Diunggah' : 'Pemberkasan Fisik'}</span>)}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 dropdown-container">
                          <div className="relative">
                            <button onClick={() => setActiveDropdownId(activeDropdownId === dropdownKey ? null : dropdownKey)} disabled={disableAction} className={`p-3 rounded-xl border transition-all ${disableAction ? 'opacity-50 cursor-not-allowed' : ''} ${activeDropdownId === dropdownKey ? 'bg-blue-600 border-blue-600 text-white shadow-xl' : 'bg-white text-slate-300 border-slate-100 hover:border-blue-400 hover:text-blue-600'}`}><MoreHorizontal size={20} /></button>
                            {activeDropdownId === dropdownKey && (
                              <div className="absolute right-0 top-12 w-60 bg-white rounded-[1.5rem] shadow-2xl border border-slate-50 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                <div className="p-2 space-y-1">
                                  {hasFile && isOnline && data && (
                                    <>
                                      <button onClick={() => handleActionFile(data.file_url, 'docseminar', false)} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-slate-600 hover:bg-slate-50 rounded-xl transition uppercase tracking-widest"><Eye size={16} className="text-slate-400" /> Lihat Berkas</button>
                                      <button onClick={() => handleActionFile(data.file_url, 'docseminar', true)} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black text-slate-600 hover:bg-slate-50 rounded-xl transition uppercase tracking-widest"><Download size={16} className="text-slate-400" /> Unduh PDF</button>
                                      <div className="h-px bg-slate-50 my-1 mx-2" />
          ...
          <div className="bg-white rounded-[2rem] p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-100">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 pt-4">
                ...
                <button onClick={() => handleVerify(docToReject!.id, 'Ditolak', docToReject!.category, rejectReason)} disabled={!rejectReason.trim() || processingDoc === `${docToReject?.category}_${docToReject?.id}`} className="flex-1 py-3.5 px-4 bg-red-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-700 active:scale-95 transition-all shadow-lg shadow-red-200 disabled:opacity-50 disabled:cursor-not-allowed">
                  {processingDoc === `${docToReject?.category}_${docToReject?.id}` ? 'Memproses...' : 'Kirim Penolakan'}
                </button>
              </div>
            </div>
          </div>
