...
const sidangDocsList = [
  ...
  { id: 'matriks_perbaikan', label: "Formulir Matriks Perbaikan Skripsi (TTD)", type: 'online' },
  ...
  { id: 'biodata_sidang', label: "Biodata Sidang Diserta Ijazah SMA & Akte", type: 'online' },
  { id: 'foto_sidang', label: "Foto Hitam Putih Cerah ukuran 2x3 (6 Buah)", type: 'online' },
  { id: 'skripsi_tugas_akhir', label: "Skripsi Tugas Akhir (File PDF)", type: 'online' },
  { id: 'flyer_skripsi', label: "Flyer Skripsi Tugas Akhir (File PDF)", type: 'online' },
  { id: 'print_skripsi', label: "Print Out Skripsi 5 buah belum dijilid", type: 'offline' },
  { id: 'flyer_a3', label: "Flyer Skripsi ukuran A3 (Cetak Fisik)", type: 'offline' },
];
...
export default function DokumenSidangMahasiswa() {
  ...
  const handleUploadFile = async (file: File, docId: string, docLabel: string) => {
    ...
    try {
      ...
      const filePath = `${userId}/sidang_${docId}_${Date.now()}.pdf`;
      ...
      if (tendik) {
        await sendNotification(
          ...
          `Mahasiswa atas nama ${mhsName} telah mengunggah berkas ${docLabel}. Mohon segera diverifikasi.`
        );
      }
      
      mutate(); 
      alert(`Berkas ${docLabel} berhasil diunggah. Mohon tunggu verifikasi Tendik.`);
    } catch (err: any) { 
  ...
  const handleDeleteFile = async (docId: string, path: string) => {
    if (!window.confirm("Hapus file dokumen ini dan unggah ulang?")) return;
    try {
      setUploading(true);
      await supabase.storage.from('docseminar').remove([path]);
      await supabase.from('sidang_documents_verification').delete().match({ proposal_id: proposalId, nama_dokumen: docId });
      mutate(); 
    } catch (err) { 
      console.error(err); 
    } finally { 
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
          <div className="max-w-6xl mx-auto w-full">
            <div className="mb-10">
               ...
               <p className="text-slate-500 font-medium mt-2">Daftar ceklis dokumen yang harus diunggah dan diserahkan ke Tendik untuk keperluan Sidang Skripsi.</p>
            </div>
                        ...
                        <div key={doc.id} className={`flex items-center justify-between p-6 rounded-[2rem] border transition-all duration-300 ${isComplete ? 'bg-emerald-50/40 border-emerald-100' : isPending ? 'bg-blue-50/30 border-blue-100' : 'bg-white border-slate-100 hover:border-slate-200'}`}>
                          <div className="flex items-center gap-6">
                            ...
                            <div>
                              ...
                              <p className={`text-[10px] font-bold uppercase tracking-widest ${doc.type === 'offline' ? 'text-indigo-500' : 'text-slate-400'}`}>
                                {doc.type === 'offline' ? 'Pemberkasan Fisik (Manual)' : 'Dokumen Digital (Upload)'}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-3">
                            {doc.type === 'online' && (
                              <div className="flex items-center gap-2">
                                {!documentStatus?.file_url ? (
                                  <label className={`px-6 py-2.5 bg-blue-600 text-white text-[9px] font-black uppercase tracking-widest rounded-xl transition-all shadow-md flex items-center justify-center cursor-pointer hover:bg-slate-900 ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                                    ...
                                    <input 
                                      ...
                                      accept=".pdf" 
                                      onChange={(e) => {
                                        if (e.target.files?.[0]) handleUploadFile(e.target.files[0], doc.id, doc.label);
                                        e.target.value = ''; 
                                      }} 
                                    />
                                  </label>
                                  ...
                                  <button
                                      type="button"
                                      onClick={() => handleOpenFile(documentStatus.file_url!)}
                                    ...
                                    {!isComplete && (
                                      <button 
                                        onClick={() => handleDeleteFile(doc.id, documentStatus.file_url!)} 
                            ...
                            <div className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border ${
                              ...
                              {isComplete ? 'Telah Diverifikasi' : isRejected ? 'Ditolak' : isPending ? 'Menunggu Verifikasi' : 'Belum Diserahkan'}
                            </div>
                          </div>
                        </div>
                      );
